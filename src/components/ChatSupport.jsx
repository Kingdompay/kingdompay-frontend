      ...
      <div className="w-full max-w-md md:max-w-6xl bg-white md:my-8 md:rounded-3xl md:shadow-2xl min-h-screen md:min-h-[800px] flex flex-col md:flex-row overflow-hidden relative">
        ...
        <div className="md:w-1/3 lg:w-1/4 bg-white md:border-r md:border-gray-100 flex flex-col">
          <header className="sticky top-0 z-10 p-4 bg-white md:bg-transparent">
            <div className="flex justify-between items-center">
              ...
              <h1 className="text-lg font-bold text-[#1A3F22] m-0">Chat Support</h1>
        ...
        <main className="flex-grow p-4 pb-28 md:pb-8 overflow-y-auto bg-gray-50 md:bg-white">
          <div className="max-w-3xl mx-auto animate-fade-in-up space-y-8">
            ...
            <section className="flex flex-col gap-4">
              {messages.map(msg => (
                <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-xs p-3 rounded-xl ${msg.sender === 'user' ? 'bg-[#6f9c16] text-white' : 'bg-[#f3f4f6] text-[#1A3F22]'}`}>
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="bg-[#f3f4f6] p-3 rounded-xl text-[#6b7280]">
            ...
            {/* Quick Replies */}
            <section>
              <h2 className="text-base font-semibold text-[#1A3F22] mb-2">Quick Replies</h2>
              <div className="flex flex-wrap gap-2">
                {quickReplies.map((reply, idx) => (
                  <button key={idx} onClick={() => setMessage(reply)} className="bg-white border border-[#e5e7eb] rounded-md px-3 py-1 text-sm text-[#1A3F22] hover:bg-gray-50 transition-colors">
                    {reply}
                  </button>
                ))}
              </div>
            ...
            <section>
              <div className="flex items-center gap-2">
                <input type="text" value={message} onChange={e => setMessage(e.target.value)} onKeyPress={e => e.key === 'Enter' && sendMessage()} placeholder="Type your message..." className="flex-1 py-2 px-4 border border-[#e5e7eb] rounded-full bg-[#f9fafb] focus:outline-none" />
                <button onClick={sendMessage} disabled={!message.trim()} className="w-10 h-10 rounded-full bg-[#6f9c16] text-white flex items-center justify-center disabled:bg-[#d1d5db]">
